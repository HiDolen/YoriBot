...
clear_plugin_info = on_command("清理插件信息", permission=SUPERUSER, priority=2, block=True)
clear_group_info = on_command("清理群信息", permission=SUPERUSER, priority=2, block=True)

@clear_plugin_info.handle()
async def _(bot: Bot):
    logger.info("超级用户主动清理插件信息……")
